...
function print(diff, options) {
  ...
  if (details.length > 1) {
    msg = 'Coverage delta: \n' + details.map(function(what) {
        options.next = 0;
        out = {};
        return getMsg(print_item(diff, what, options), sep);
      })
...
function print_item(diff, what, options) {
  ...
  if (!item || !(what in item)) {
    return next(options.brief ? null : 'No coverage difference in ' + what);
  }
  ...
  function _out(metric, metricName) {
    var
      ...
      simpleTmpl = (metric.pct ?
        c(`Coverage ${dir} ${sign}${metric.pct}% (${metric.covered}) ${metricName}.`) :
